...
export const SteamShader: React.FC<SteamShaderProps> = ({
  ...
  const fragmentShader = `
    ...
    uniform float uSpeed,uDensity,uBrightness,uAlpha,uHue,uVertical;
    uniform vec2 uMaskCenter;
    uniform float uMaskRadius,uMaskHeight,uMaskFeather,uCutoutOffset,uCutoutRadius;

    float hash(vec2 p){ return fract(sin(dot(p, vec2(127.1,311.7))) * 43758.5453123); }
    float noise(vec2 p){ vec2 i=floor(p), f=fract(p); vec2 u=f*f*(3.0-2.0*f);
      return mix(mix(hash(i+vec2(0,0)), hash(i+vec2(1,0)), u.x), mix(hash(i+vec2(0,1)), hash(i+vec2(1,1)), u.x), u.y);
    }
    float fbm(vec2 p){ float v=0.0,a=0.5; for(int i=0;i<5;i++){ v+=a*noise(p); p=mat2(1.6,1.2,-1.2,1.6)*p+3.0; a*=0.55;} return v; }
    ...
    void main(){
      ...
      vec2 p=uv*vec2(1.,2.) + vec2(0., -t*1.5);
      float w1=fbm(p*1.25+vec2(0.,-t*0.5));
      ...
      vec2 uv01=vUv; float cx=uMaskCenter.x, cy=uMaskCenter.y; float r=uMaskRadius, h=uMaskHeight, f=max(uMaskFeather,0.0001);
      float d=length(uv01-vec2(cx,cy)); float insideCircle=smoothstep(r+f, r-f, d); float belowCenter=smoothstep(-f, f, cy-uv01.y);
      float semiMask=insideCircle*belowCenter;
      float dx=abs(uv01.x-cx)-r; float insideX=smoothstep(0.,f,-dx); float aboveBottom=smoothstep(-f,f,uv01.y-cy); float belowTop=smoothstep(-f,f,(cy+h)-uv01.y);
      float rectMask=insideX*aboveBottom*belowTop; float mask=clamp(max(semiMask,rectMask),0.0,1.0);
      vec2 faceC=vec2(cx, cy - uCutoutOffset); float faceD=length(uv01-faceC); float faceHole=smoothstep(uCutoutRadius-f, uCutoutRadius+f, faceD); mask*= (1.0 - faceHole);
      float distNorm=d/max(r,1e-5); float gradCircle=1.0 - smoothstep(0.2,1.0,distNorm); float yDist=clamp((uv01.y-cy)/max(h,1e-5),0.,1.); float gradRect=1.0 - smoothstep(0.0,1.0,yDist);
      float gradient=clamp(max(gradCircle,gradRect),0.0,1.0);

      vec3 col=hsv2rgb(vec3(uHue,0.02,0.9))*uBrightness; vec3 rgb=mix(vec3(0.0), col, steam*mask*gradient);
      float a=steam*uAlpha*mask*gradient; gl_FragColor=vec4(rgb,a);
    }
  `;
  ...
  return (
    <ShaderCanvas
      ...
      onBeforeFrame={(u) => {
        ...
        if (u.uMaskCenter) { const c = maskCenterRef.current; u.uMaskCenter.value.set(c.x, c.y); }
